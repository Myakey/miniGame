...
export class BlokM extends Phaser.Scene{
  ...
  generateMap(){
    ...
    const groundLayer = map.createLayer("Ground", [mainTileset, LibraryBlokM]);
    const wallLayer = map.createLayer("Wall", mainTileset)
  ...
  showDialog(text) {
  const dialogBox = this.add.text(100, 100, text, {
    fontSize: '16px',
    fill: '#fff',
    backgroundColor: '#000'
