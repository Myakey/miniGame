...
export default function DialogueBox({ speaker,text, onNext, autoPlay, setAutoPlay }){
    // const [isAutoPlaying, setIsAutoPlaying] = useState(autoPlay);
    // const [autoInterval, setAutoInterval] = useState(null);

    // useEffect (() => {
    //     if (isAutoPlaying) {
    //         const interval = setInterval(() => { onNext();}, 2500); //harusnya 2,5 detik per dialog
        ...
        <>
            <div className="dialogue-box fixed bottom-10 left-1/2 transform -translate-x-1/2 max-w-3xl w-[90%] px-6 py-4 rounded-[2rem] bg-white/90 shadow-lg backdrop-blur-md border border-gray-300 cursor-pointer"
                ...
                <div className="absolute top-2 right-6 flex gap-2">
                    <button onClick={(e) => {e.stopPropagation(); alert("LOG button is not implemented yet.");
                        }} className="text-xs bg-gray-300 px-3 py-1 rounded-full shadow hover:bg-gray-400">
                            LOG
                    </button>
                    <button onClick={(e) => {e.stopPropagation(); setAutoPlay(!autoPlay);
                        }} className={`text-xs px-3 py-1 rounded-full shadow ${autoPlay ? "bg-green-400" : "bg-gray-300 hover:bg-gray-400"}`}>
                            AUTO
                    </button>
                </div>
            </div>
